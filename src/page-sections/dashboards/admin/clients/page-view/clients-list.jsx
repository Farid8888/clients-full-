...
const ClientsListPageView = () => {
  ...
  let filteredUsers = stableSort(users, getComparator(order, orderBy)).filter(item => {
      if (userFilter.role)
          ...
          return item.userName.toLowerCase().includes(userFilter.search.toLowerCase()) ||
              ...
              item.phoneNumber.toLowerCase().includes(userFilter.search.toLowerCase());
      else return true;
  });
  ...
  }, [clientsList, firstFetch, updateClientsList]);
        ...
        <TableContainer>
          <Scrollbar autoHide={false}>
            <Table>
              ...
              <TableBody>
                {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(user => <UserTableRow key={user.id} user={user} isSelected={isSelected(user.id)} handleSelectRow={handleSelectRow} handleEditClient={handleEditClient} />)}
...
export default ClientsListPageView;