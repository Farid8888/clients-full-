...
const HeadingArea = ({
  ...
  return <FlexBetween flexWrap="wrap" gap={1}>
      <FlexBox alignItems="center">
        ...
        <Paragraph fontSize={16}>{t("Traders")}</Paragraph>
      </FlexBox>

      <TabContext value={value}>
        <TabListWrapper variant="scrollable" onChange={changeTab}>
          <Tab disableRipple label={t("All Traders")} value="" />
        </TabListWrapper>
      </TabContext>

      <Button variant="contained" startIcon={<Add />} onClick={() => navigate("/dashboard/admin/add-user?role=Trader")}>
        {t("Add New Trader")}
      </Button>
    </FlexBetween>;
};
